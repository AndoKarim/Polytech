...
int main(int argc,char **argv){
  ...
  //Le dernier argument n'est pas un repertoire
...
for(i=optind;i<argc-1;i++){
  ...
  if (is_Dir(argv[i])){
      //Et que l'option recusrvive est la 
    if (recursive==1){
      //On copie l'intérieur du rep dans le rep de destination
